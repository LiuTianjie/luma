...
RUN --mount=type=cache,id=lae-uv-v1,target=/root/.cache/uv \
    ...
    uv sync --frozen --no-dev --no-editable --no-build-isolation --package lae-api && \
    uv sync --frozen --no-dev --no-editable --no-build-isolation --package lae-store --extra migrations --inexact
