...
本清单以重构前版本 v0.1.299 与当前正式 Dashboard 为对照。用于记录现有能力的新入口及验证边界，不代表所有操作已在生产执行。路由均相对 `/dashboard`；对象名按单个 URL segment 编码。

## 验证口径

- **代码核对**：对照旧入口、事件处理器和新页面挂载条件，确认调用链保留；不等同于 API 或浏览器实测。
- **SSR**：`node --test dashboard-src/tests/infrastructure.cjs` 覆盖分区、详情字段单位与页面语义；现有业务 panel 在该测试中替身化，仅证明分区选择和页面语义。
- **真实运行待验**：需在新版 Manager 的浏览器/API 环境实际验证；发布负责人补充结果。涉及真实发布、重启、回滚、清理、通知应选定测试对象，不能用读取页面成功替代。

## 页面加载边界

部署记录、设置、Registry 和 LAE 页面只依赖各自的分页或配置接口，登录后可以在 `/v1/dashboard` 聚合快照完成前先渲染。设置页使用一次性的 `GET /v1/control-resources` 配置快照返回 Secret 名称、Registry、Git Provider 和存储类摘要；原有四个资源接口仍保留给 CLI 和兼容调用。总览、应用、节点、存储和需要运行时节点/服务数据的页面继续等待 `/v1/dashboard`，避免在独立页面重复加载构建与部署历史。
...
inline Shell 不挂载 `OverlayShell`，不声明模态，不全局截获 Escape。WebSocket 仍通过 `/v1/terminal/browser`，仍发送鉴权、resize、input、close；卸载会关闭已连接或尚在连接的 socket，旧事件回调不会写入已销毁终端。刷新页面会重新建立会话，不承诺恢复原交互进程。v0.1.302 已通过 Manager 节点及 word2pdf_web 容器的真实 WebSocket 输入输出验证；每次终端改动后仍需复查连接、尺寸与退出。
...
更新与创建工作台使用单一页面标题、限定宽度、独立编辑器和按内容高度排列的摘要；应用详情移除旧弹窗的尺寸及内部滚动约束。浏览器验证 Compose 两个文件分别编辑后往返切换，内容均保留；未执行部署。通知渠道表单使用真实 Manager 的只读告警接口验证加载，未保存配置或发送消息。

## 页面布局回归范围

针对线上 v0.1.302 逐页检查发现的布局问题，v0.1.303 修复日志横向撑宽、服务表格镜像覆盖状态列、配置 YAML 换行和对比度、默认按钮黑底黑字、历史详情属性堆叠、Git 构建操作栏遮挡输入、终端返回按钮撑满整行，以及节点详情资源单位。

回归时须等待业务数据加载完成，并检查实际内容区域；仅检查 body 的 scrollWidth 会漏掉被父容器裁切的内容和表格列间覆盖。检查入口包括：

- 总览、应用列表；应用概览、服务列表/详情、日志、指标、配置、版本、更新。
- 创建来源选择、镜像、YAML、模板和 Git 构建；构建及部署历史、两类详情与事件分页。
- 告警事件、指标、日志、规则、新建规则、渠道、新建渠道及发送记录。
- 节点列表、加入节点、区域、网络、系统维护、节点详情；节点和容器 Shell。
- 存储卷与存储类、容量与回收；镜像列表、详情、保留策略、清理队列及删除确认。
- Secret、Registry、Git 凭据的列表与新建/轮换页、存储配置；LAE 六个资源标签。

长 ID、镜像 digest 和路径必须能完整阅读；代码保留换行，表格和日志在自身区域滚动。桌面宽度通过不代表窄屏通过；改动后需复查两种宽度。页面浏览不等同于执行真实重启、回滚、删除或发送通知。
