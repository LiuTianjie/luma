...
4. S3-compatible adapter 只能写入 LAE 计算的
   ...
   path/URL 不参与 object key。adapter 必须先写私有 validation，验证后原子发布；
...
CLI 级 host allowlist 不能约束 registry 返回的 token-service/CDN redirect，因此生产开放前仍要由 builder 网络层实现 DNS/IP/redirect egress enforcement，并通过真实 validation 验证；当前实现不能据此宣称 production-ready。
...
当前协议从私有对象存储读取 baseline/candidate 两份已验真的 DeploymentPlan，向 Operation
只投影 service、public HTTP route、persistent volume 与 environment 四类
`added/removed/changed` key，不公开镜像、命令、secret 值、对象 key 或源码。稳定确认代码为
`SERVICE_REMOVAL`、`PUBLIC_ROUTE_CHANGE`、`PERSISTENT_VOLUME_CHANGE` 和
`REQUIRED_ENVIRONMENT_ADDED`。Web/CLI 必须取得用户明确批准；deployment admission 会把
提交集合与 candidate analysis 所属 check-update Operation 的要求做精确比对。旧版只有
digest、没有差异明细的结果必须重新检查，不能直接部署。
...
每个 adapter/policy 变更必须跑 golden plan snapshot、Luma preview、validation 真部署和失败清理测试。
