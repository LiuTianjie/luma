...
更新检查当前会从数据库中已保存的 source/ref/connection 复制新 source revision，不接受调用者注入 repository/ref/connection，并复用 analysis Worker；它不会自动替换健康部署。DeploymentPlan 的稳定摘要只包含 source tree/commit/policy 等语义输入，不包含每次 fetch 生成的 `sourceSnapshotId`/`planId` 等尝试 ID。真实 staging 中两次独立 check-update 生成同一 candidate digest `sha256:2632aebaa16e9fa65fff706af46835c67d26f7e60d79329316df8947e9a4b804`；用该 analysis 重部署后再次检查，baseline/candidate source 与 plan digest 均相等，四个变化字段正确收敛为 false。env/route/volume 破坏性变更分类、逐项展示、人工确认和确认后创建 deployment 仍是下一阶段门槛。
