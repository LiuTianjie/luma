...
- **发布与代码 gate：** 已发布的 Luma `0.1.171` 曾以 719/719 项测试通过 gate，当前 live CLI、Control 与 fleet 均为 `0.1.171`；LAE 当前 working tree 的 `make check` 为 355 项通过、23 项条件集成测试跳过，身份/邮件定向 29 项、Luma 部署资产 10 项与 Web production build 也通过。该 working tree 另含尚未发布的 preview-auth 和 rollout/route 可靠性修复，必须形成新不可变 release 才能视为 live。
...
- **产品 E2E：** Mailpit 捕获的注册 challenge、默认 deploy token、CLI 登录/身份/模板/套餐/应用、模板 launch 与 analysis 已执行；Mailpit 不向用户真实邮箱投递，最新 AI provider-backed 四态 verdict 也尚未形成完整验收记录。租户 source → Builder → Runtime 部署、随机域名、观测和 lifecycle 动作矩阵仍待最终 E2E；下述 production blockers 继续有效。
- **P0 可用性：** 已复现控制面升级、Docker 配置变更或其他应用发布后，既有 route 出现 404/502 且需重建 allocation 才恢复。当前 live `0.1.171` 不能视为已关闭；发布门槛见 [SOP 11.1](./10-operations-troubleshooting-sop.md#111-控制面升级或其他应用部署后批量-404502) 和 [部署手册](./11-deployment-and-upgrade.md)。
...
| Upgrade/route continuity | In progress | 配置无变化不得重启 Docker；必须区分 router 缺失的 404 与 upstream 断链的 502/504；提交结果需绑定本次 `JobModifyIndex`、evaluation、deployment、JobVersion 和新 allocation health；升级/发布后还要验证未参与变更的 sentinel routes。当前 live 仍有批量 404/502 P0，不能以手工重启作为完成证据 |
...
| 2026-07-12 | Live/version 与文档交叉审计 | Luma CLI/Control/live fleet 已统一为 `0.1.171`；8 节点 ready，LAE 9 task 运行，Web/API/Agent/artifact/Control 探针 200，Agent AI configured。补充原始需求证据矩阵，并把真实邮箱、provider-backed verdict、tenant Runtime E2E 与批量 404/502 P0 保持为未完成 | `luma version`、`luma status --format json`、`luma history lae-platform-staging --format json` 与只读 HTTPS probes；未执行部署 |
