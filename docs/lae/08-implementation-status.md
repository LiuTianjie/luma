...
- **发布与代码 gate：** Luma `0.1.192` 以 757/757 项测试通过 gate，当前 live CLI、Control 与 manager agent 均为 `0.1.192`；其余在线 agent 为 `0.1.175`-`0.1.188`，fleet 并未统一。LAE Web commit `fcff4c8` 的 scaffold、TypeScript 与 production build 已通过，immutable image digest 已部署到平台 Job version 18。
...
- **产品 E2E：** FastAPI 模板已真实走通 launch → Agent verdict → Builder build → Runtime deploy → random domain/TLS，且无需用户提供 Luma 文件；当前两个租户应用均健康运行在 `tecent`。Mailpit 捕获的注册 challenge、默认 deploy token 与 CLI 基础流程也已执行。HTML/ZIP、私有 Git、Compose 双 HTTP/volume、观测和全部 lifecycle 动作矩阵仍待完成，Mailpit 不代表真实邮箱送达。
- **P0 可用性：** `0.1.190` 修复跨 deployment service/router 名称碰撞，`0.1.192` 让 edge service 通过节点 `luma_tailscale_ip` metadata 注册可达 upstream。多次 manager 升级、全新租户发布与 LAE Web Job v18 更新期间，两条既有租户 route 连续返回 200，无人工重启。已复现的 staging 路径回归关闭；Docker restart/CNI 自愈、route reconciliation 故障注入与更广 sentinel 矩阵仍阻塞 production。
...
| Upgrade/route continuity | In progress | 配置无变化不得重启 Docker；提交结果需绑定本次 `JobModifyIndex`、evaluation、deployment、JobVersion 和新 allocation health。`0.1.190`/`0.1.192` 已修复名称碰撞与跨节点 upstream，Control/tenant/Web rollout sentinel 回归通过且无需人工重启；Docker restart/CNI 自愈、reconciliation 故障注入与更多 edge sentinel 仍未完成 |
...
| 2026-07-12 | Live/version 与文档交叉审计（历史快照） | 当次快照为 Luma `0.1.171`、8 节点 ready、LAE 9 task 运行，Web/API/Agent/artifact/Control 探针 200，Agent AI configured；该行已被下方 `0.1.192` live 记录取代，只保留审计时间线 | `luma version`、`luma status --format json`、`luma history lae-platform-staging --format json` 与只读 HTTPS probes；未执行部署 |
