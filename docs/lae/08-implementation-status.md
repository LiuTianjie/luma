...
- **发布与代码 gate：** Luma `0.1.233` 的当前候选代码通过 813 项 pytest 与 130 项 subtest；LAE 通过 403 项测试（25 项按环境跳过）、contracts、compile 和 health smoke。tag/package version mismatch gate 保持启用。CLI、Control 与 manager agent 的包版本仍为 `0.1.233`；manager Control 已运行 commit `d0ffc7a` 对应的不可变候选镜像 `100.66.177.70:5000/luma-control@sha256:dca605433652e74232ef6d08b5327c3b6342ef1aa5dd435f6f37fb3aff03d06c`。本轮尚未做 worker-wide fleet 升级，在线非 manager agent 仍主要为 `0.1.228`，离线 `blg` 按当前决策保持 `0.1.175`。LAE staging 9 个平台 task 使用 exact commit `d0ffc7a` 的平台镜像并全部运行，Nomad job version 为 50。
