...
除明确写成 `/internal/v1/*` 的管理员接口和第 13 节 Luma 内部接口外，本文件的
公开路径都**相对于上述 `/v1` 基址**。例如 `POST /auth/register` 的实际完整路径是
`POST /v1/auth/register`；客户端不得再额外拼接第二个 `/v1`。
...
默认不包含 `billing:checkout`；deploy token 协议也不开放宽泛的
`billing:write`、`tokens:write` 或 `admin:*`。可为专用 Agent token 显式授予
`billing:checkout`，它只能创建 checkout
...
`POST /auth/preview` 只在显式启用的 dev/validation preview mode 返回凭据，并只为保留的
`.invalid` 测试身份签发短时 challenge；关闭时稳定返回 404，production 必须关闭。
`GET /sessions` 与
`DELETE /sessions/{id}` 是后续的多 session 管理目标，当前没有公开路由，客户端不得
依赖。
...
实现；真实 Control principal/token file、内部 TLS 路径与 validation E2E 未完成时仍
...
`POST /analyses/{id}/rerun` 与独立的
`POST /applications/{id}/update-checks` 是目标态别名，当前没有公开路由。需要重新分析
源码时创建新的 analysis；应用更新检查使用上述 lifecycle action，并复用数据库中
保存的 source binding。
...
当前没有通用 `PATCH /applications/{id}` 或直接
`DELETE /applications/{id}`；删除必须走第 7 节的 durable `actions/delete`，应用元数据
编辑是后续目标。
...
- `GET /applications/{id}/analyses/{analysisId}/configuration`
...
首次部署与普通 analysis 的部署请求严格只包含 `analysisId` 与 `environmentVersion`。
check-update candidate 可额外携带排序、去重的 `confirmedChanges`，但只允许
`SERVICE_REMOVAL`、`PUBLIC_ROUTE_CHANGE`、`PERSISTENT_VOLUME_CHANGE` 与
`REQUIRED_ENVIRONMENT_ADDED`，并且必须与服务端计算的精确集合完全一致。Primary HTTP route、service、
...
- 配额无法预留。
- destructive update 未确认精确代码集合，返回
  `LAE_DEPLOYMENT_CONFIRMATION_REQUIRED` 与 `requiredConfirmations`。
- 历史 update-check 只有 digest、没有结构化 diff，返回
  `LAE_UPDATE_CHECK_DETAILS_REQUIRED`，必须重新运行检查。
...
- `GET /operations?applicationId={appId}&kind={kind}&before={operationId}&limit=50`
- `GET /operations/{id}`
- `GET /operations/{id}/events?after=42&limit=100`
- `POST /operations/{id}/cancel`

Operation 列表按 opaque、time-sortable Operation ID 倒序分页，响应返回
`operations/hasMore/nextCursor`。`applicationId` 可把 analysis、upload、deployment 与
lifecycle 历史收敛到同一应用；`kind` 是精确过滤。列表不是“先查全租户再在响应中过滤”：
PostgreSQL 查询会按当前凭据实际拥有的 `analyses:write/sources:write/deployments:write/apps:write`
scope 生成可见 kind predicate，未知 kind 默认不可见。单一 scope 的 deploy token 只能看到
对应历史；显式查询无权 kind 返回 403。Web/CLI 仍必须保存 create 响应中的 Operation ID，
刷新后可直接读取详情并从已保存 cursor 重放事件；列表用于找回近期历史而不是替代 ID/cursor。

当前已落地的 JSON polling 基线还包括
`GET /analyses/{id}`。Analysis 读模型只返回公开 status、五类 digest、
...
- `POST /billing/mock/orders/{id}/approve`（仅 dev/validation，交互 session）
- `POST /billing/mock/orders/{id}/complete`（仅 dev/validation）

模板、Web、CLI 和 Skill 调用同一个 checkout session API。CLI/Skill 只能生成付款 URL；套餐变更必须由人类打开并确认。

`POST /billing/portal-sessions`、真实
`POST /billing/webhooks/{provider}`、refund/reconcile 是微信/支付宝 adapter 的目标态，
当前没有 production 公开路由；mock complete 只验证同构事件处理，不代表真实 provider
已接入。
...
当前管理员切片只读。`GET /internal/v1/admin/abuse-cases` 和
`POST /internal/v1/admin/applications/{id}/suspend` 是目标态，尚无路由。未来超管 action
仍必须由 LAE 创建 operation/audit 再调用 Luma，不能绕过 LAE 直接修改 Nomad。
...
- 生命周期：legacy restart/history/rollback/remove；LAE dedicated Runtime API 与 Worker 已实现 suspend/resume/restart/rollback/delete 的受限协议、durable checkpoint、失败恢复、拓扑兼容校验和 delete retain-volume，真实 PostgreSQL/Luma validation 验证仍按实施状态门禁。
