...
| 入口 | 地址 | 用途 |
| --- | --- | --- |
| Agent | `https://lae-agent-validation.itool.tech/` | 受认证的项目诊断端点 |
...
- 邮件注册/登录协议、validation preview 登录、用户级 deploy token 的创建、轮换与撤销。
...
- Lite/Pro/Ultra entitlement、配额与月/年协议；真实支付 provider 未接入时保持 validation mock/production disabled。
...
| 租户 runtime allowlist | `manager + tecent` | 用户只看到 region，不看到节点/IP；带受管卷的 validation 应用由存储兼容性进一步固定到 `manager` |
...
2026-07-15 的最终正向产品验收使用公开 API 和 Builder，从空应用完成 preview 登录、deploy token、AI 诊断、必需环境配置、四服务 Compose 构建、双公网 HTTPS route、双受管卷、restart、suspend/resume、update check、第二次部署和 rollback；4 个服务、2 条 route 与 2 个 volume 在首次部署和 rollback 后均通过拓扑验证。故意制造 rollout 失败的破坏性测试不作为本次 validation 交付的阻塞条件；它属于后续独立故障矩阵，不应混入普通用户验收。
...
3. 使用显式 validation sidecar 与环境文件执行 `luma import`。
...
- 专用 LAE 平台节点与至少两个专用 runtime failure domain；当前共享 `manager` 只适用于 validation。
...
这些门槛不影响当前 validation 体验验证，但未关闭前不得对外宣称 production GA。
