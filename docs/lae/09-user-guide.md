...
截至 2026-07-12，Luma CLI、Control 与 manager agent 已运行 `0.1.192`，其余在线 agent 仍为异构版本；真实 staging 的 9 个平台 task、TLS 和 Web/API/Agent/artifact 探针健康。FastAPI 模板已经走通 Agent 诊断、Builder 构建、Runtime 部署、随机域名与有效 TLS，两个租户应用当前均在运行。Mailpit 注册、deploy token 和 CLI 基础流程也已跑，但 Mailpit 不向用户真实邮箱投递，其他来源、AI 四态负例、观测与全部 lifecycle 仍待最终 E2E。因此本指南仍是 staging 使用说明，不是生产 GA 承诺。
...
本地完整 staging Compose 已验证所有平台服务 healthy，并走通注册邮件、默认 deploy token、application/catalog/admin、mock billing、CLI 以及 MinIO 私有 S3/CORS；日志扫描为 0 个 secret pattern、0 个 traceback。在此基础上，真实 Luma staging 已完成 import，平台固定 `lab`，tenant runtime pool 为 `manager + tecent`。FastAPI 模板的完整纵向正向链路和随机域名/TLS 已通过，当前两个租户应用健康运行；这仍不能替代其他来源、Compose 多服务、生命周期、安全负例与故障恢复验收，也不能据此宣称生产可用。
