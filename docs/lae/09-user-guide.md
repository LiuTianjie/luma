...
| 待 staging 验证 | 尚未在真实隔离 Luma staging 完成端到端、故障和安全验证 |
...
flowchart LR
    ...
    inspect -->|needs_configuration| env["补齐环境变量"]
    env --> deploy["确认并部署"]
    inspect -->|deployable| deploy
    inspect -->|not_deployable| stop["修复 blocker 后重新诊断"]
...
- `needs_configuration`：拓扑可以部署，但缺少必需环境变量；补齐后使用同一 analysis。
- `not_deployable`：存在策略 blocker；必须修改源码或 Compose 后创建新的 analysis，不能手改 LAE 生成的 Luma 文件。
...
登录建立 HttpOnly Session。正常再次登录不会重新显示已有 token。当前 Web 已实现默认 token 的首次展示；完整 token 列表、新建、轮换和撤销页面仍是发布门禁，丢失 token 时应通过受控支持流程处理。

### 4.2 从模板开始

首页模板以“湖面”形式呈现。悬停可查看模板栈和说明，点击后会：
...
当前目录包含 Next.js、FastAPI、Flask 和 Express 类型的固定版本模板。模板目录和一键诊断已实现；每日真实构建 smoke、自动下架和生产发布仍需 staging 证据。
...
应用列表显示产品级状态、稳定域名和最近更新时间。应用的物理节点名与 IP 不显示；底层节点可能因容量、维护或故障发生变化。
...
生产 API 默认是 `https://lae-api.itool.tech/v1`。访问 staging 时必须显式设置完整 `/v1` 地址：

```bash
export LAE_API_URL=https://lae-api-staging.itool.tech/v1
```
...
返回 `needs_configuration` 时，退出码为 4；这不是模板构建失败，应按下一节补配置。
...
每个动作返回新的 Operation，随后用 `lae operation watch` 观看。回滚和删除必须先展示目标 deployment、环境/route/volume 差异并取得人工确认。删除默认保留受管 volume；数据删除需要独立、明确且可审计的策略，不能从 CLI 的普通删除动作推导。
...
注册时生成的默认 deploy token **故意不含** `billing:checkout`。下面的 CLI 命令只适用于用户通过 Web Session 创建的、显式包含该 scope 的独立 token；当前 token-management/checkout Web UI 尚未完成，因此普通用户的完整自助支付仍是发布门禁。不要为了方便给默认 token 扩权。
...
Lite 是注册后的默认免费 entitlement，不需要购买。`mock` 只允许 dev/staging；用户必须自己打开 checkout URL 并确认，Agent 不得替用户完成支付。生产当前使用 `disabled` driver，微信/支付宝真实 adapter、退款和 provider sandbox 通过前，付费端点应返回不可用而不是伪成功。
...
- `needs_configuration` 时只向用户说明变量名称和原因；
- `not_deployable` 时停止，不手改平台生成的 manifest；
...
本地完整 staging Compose 已验证所有平台服务 healthy，并走通注册邮件、默认 deploy token、application/catalog/admin、mock billing、CLI 以及 MinIO 私有 S3/CORS；日志扫描为 0 个 secret pattern、0 个 traceback。这是本地 Docker Compose 证据，**不是**真实 Luma staging deployment，当前不能据此宣称线上 staging 或生产可用。

以下能力已有实现，但在真实 Luma staging 证据完成前仍不能宣称生产可用：

- HTML/ZIP、私有 Git、Builder analyze/build、runtime deploy 的完整纵向 E2E；
- lifecycle API/Worker 与 PostgreSQL 17 migration-backed 集成已验证；suspend/resume/restart/rollback/delete 的真实 Luma staging 全场景仍待验收；
- placement 正向 allowlist、容量和 admin“调度位置”视图已实现；真实 allocation 关联、节点故障重调度和 volume affinity 演练仍待完成；
- wildcard DNS/TLS、多 HTTP route 和失败保旧版本；
