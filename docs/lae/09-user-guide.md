...
截至 2026-07-12，Luma Control、manager agent 与 6 个在线非 manager agent 已运行 `0.1.196`；离线 `blg` 待恢复后补升级。真实 staging Job v21 的 9 个平台 task、TLS 和 Web/API/Agent/artifact 探针健康。FastAPI 模板已经走通 Agent 诊断、Builder 构建、Runtime 部署、随机域名与有效 TLS；CLI 的更新检查、NDJSON 进度、同 revision 重部署、日志/指标与主要 lifecycle 操作也已跑通。Mailpit 注册、deploy token 和 CLI 基础流程已有证据，但 Mailpit 不向用户真实邮箱投递，HTML/ZIP、私有 Git、Compose 双 HTTP/volume 与完整安全负例仍待最终 E2E。因此本指南仍是 staging 使用说明，不是生产 GA 承诺。
...
本地完整 staging Compose 已验证所有平台服务 healthy，并走通注册邮件、默认 deploy token、application/catalog/admin、mock billing、CLI 以及 MinIO 私有 S3/CORS；日志扫描为 0 个 secret pattern、0 个 traceback。在此基础上，真实 Luma staging Job v21 已完成 import，平台固定 `lab`，tenant runtime pool 为 `manager + tecent`。FastAPI 模板、随机域名/TLS、稳定更新检查、同 revision 重部署、日志/指标和 suspend/resume/restart 已通过；Control、manager 和全部在线 agent 已升级到 `0.1.196`，升级后的 240 次定向外部探针为 0 failure。离线 `blg` 待补升级；其他来源、Compose 多服务、安全负例与故障恢复验收仍不能由这些证据替代，也不能据此宣称生产可用。

以下能力已有实现或已具备真实 staging 部署基础，但在对应 E2E 证据完成前仍不能宣称生产可用：

- FastAPI 模板单 HTTP 已通过；HTML/ZIP、私有 Git、Compose 双 HTTP/volume 的完整纵向 E2E 仍待完成；
- lifecycle API/Worker 与 PostgreSQL 17 migration-backed 集成已验证；suspend/resume/restart 已跑真实 staging，rollback/delete 和各动作故障恢复仍待验收；
- placement 正向 allowlist、容量和 admin“调度位置”视图已实现；真实 allocation 关联、节点故障重调度和 volume affinity 演练仍待完成；
- 单 HTTP 随机域名/TLS 已通过；多 HTTP route、域名稳定性和失败保旧版本仍待完成；
