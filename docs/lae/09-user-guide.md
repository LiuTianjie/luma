...
截至 2026-07-14，Luma CLI、Control 与 manager agent 包版本为 `0.1.233`，Control 运行 runtime-observer 候选，LAE staging 运行 exact ref `c7873e6caf08344e683da9d9c2992445792bab55`（Nomad job v53）；本轮未对非 manager agent 做全量升级。真实 staging 的 9 个 LAE service、wildcard DNS-01 TLS 和 Web/API/Agent/artifact 探针健康。四服务 Compose 已走通 Agent 诊断、必需环境配置、Builder 构建、超过旧 3 分钟窗口的首次冷拉、双公网 HTTPS route、双持久卷、restart、suspend/resume、更新检查、明确 unsupported blocker 与删除；部署界面可消费 build/render/volumes/runtime/verify 的真实事件。四个模板均完成真实部署 smoke，平台还会每日重跑，连续三次失败自动下架，下一次成功自动恢复。clean-room Agent 也已仅凭项目内 Skill、`lae` CLI 和 deploy token 完成模板部署、deployment history、重启与清理。ZIP 与真实私有 Git也已分别完成公开 API 全链路，私有 Git覆盖 Worker 重启恢复。preview 不代表真实邮箱送达，完整安全负例仍待最终 E2E。因此本指南仍是 staging 使用说明，不是生产 GA 承诺。
...
本地完整 staging Compose 已验证所有平台服务 healthy，并走通 preview auth、默认 deploy token、application/catalog/admin、mock billing、CLI 以及 MinIO 私有 S3/CORS；日志扫描为 0 个 secret pattern、0 个 traceback。在此基础上，真实 Luma staging 的 9 个平台 task 当前在 `manager`，tenant runtime pool 为 `manager + tecent`，构建和内部 registry 在 `builder`。`0.1.233` 已通过四服务 Compose 双 HTTPS route/双持久卷、ZIP、真实私有 Git的完整产品验收和 clean-room CLI/Skill 验收；CLI、Control、manager agent 为 `0.1.233`，非 manager fleet 未在本轮全量升级。真实邮件、安全负例、rollback/失败保旧和完整数据恢复验收仍不能由这些证据替代，也不能据此宣称生产可用。
