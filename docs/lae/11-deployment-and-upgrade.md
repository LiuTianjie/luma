...
validation bundle 的 `LAE_AGENT_CONTROLLER_TOKEN` 由 Luma secret 注入平台侧
...
并由 Luma secret 注入；当前 validation 将 ARK Key/Model 映射到这三个通用变量。启用 AI 后 runner
需要 bridge egress 访问 HTTPS controller，生产应在宿主防火墙或 egress
proxy 收紧为 controller-only。

validation 当前允许 Builder 通过 HTTPS + scoped static token 访问独立 controller
...
本轮外部模型增强仅作为 validation 能力：controller 加载版本化 LAE Knowledge Pack，且只接收确定性结构化
...
4. 由 Luma Builder 从同一 Git ref 构建 LAE 平台镜像，并用显式 validation sidecar import/deploy。

租户应用实际落点由 Luma 内部决定。validation 当前通过
...
本文所有写操作都要求已批准的 validation 变更窗口。当前集群以 `manager` 为唯一
控制面，且 manager 本身也显式承担 validation runtime；`aly` 是已过时、必须跳过的
历史名称，不属于本轮升级目标。平台 validation 当前落在 `manager`，租户 runtime 候选为
`manager + tecent`，构建与内部 registry 均在 `builder`。专用 production
本地盘容量/异机恢复和 runner pool 仍是门禁；
未关闭时不要把 validation 步骤改名后当作 production 发布。
...
运行 `100.66.177.70:5000/luma-control:sha-028be71`，实际 digest 为 `sha256:995a524c9c624fcff7478094fd246312d02618b79e5c085ef81f08caab06b8ee`；LAE validation 使用 exact ref
...
sentinel、rollback/失败保旧与数据恢复仍是 production gate；ZIP 和真实私有 Git正向链路已通过，Mailpit/preview 仍不能
证明真实邮箱送达。
...
- 构建配置必须与本次 live `luma build config` 完全一致；当前 target pull 与
  ...
  不能继续从旧文档复制；
- production sidecar `lae/deploy/luma/luma.compose.yml` 不参与 validation import。

预发布 ref 示例（这是 Git 写操作，只在候选 commit 已评审后执行）：

```bash
git tag --annotate "$CANDIDATE_REF" "$FULL_SHA" \
  --message "LAE validation candidate $FULL_SHA"
...
## 5. 生成并安装 validation 配置包

先为 tenant volume 注册 validation 专用定义；租户卷 host 与 eligible runtime 都固定为
...
必须把不可变 validation tag 写成完整 tag ref，让旧 installer 正确取 archive：
...
## 7. 导入并部署 LAE validation

`luma import` 会真实 clone、build、push 和 deploy，没有 preview-only 模式。
本轮真实 validation 已用该链路完成 import；重跑仍是写操作，只能在前述门禁全部
通过后执行。当前 builder 直连外网比 manager egress 稳定，因此 validation 命令必须
...
`direct` 是本次 validation 的已验证选择，不是所有环境的永久默认；如果 production
...
实时 Nomad allocation 关联，不能把代码测试视为 validation 证据。
...
| 首次 validation 部署失败且无旧版本 | 移除失败 deployment，默认保留 storage | 禁止普通回退使用 `--delete-storage` |
...
- 真实 SMTP 已接入；payment 在 provider 未完成前继续 `disabled`，不能把 mock
  带到 production；
- 完整 validation E2E、故障注入、回滚、取消、GC 与审计通过。
...
5. validation 先升级 manager，再升级 agents，再显式 sidecar import；
