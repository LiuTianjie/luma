...
截至 2026-07-11，真实 Luma staging 尚未部署。上面的公网探针是部署后的 runbook，不是“这些 URL 当前应当在线”的现状声明。
...
3. staging 检查 Mailpit task health和 API 到 `mailpit:1025` 的内部连接；Mailpit 没有公网 route。
4. production 检查 SMTP host/465/TLS、发件人、provider 状态、退信/限流、SPF/DKIM/DMARC 和 DNS。
