...
截至 2026-07-14，Luma CLI、Control、manager agent 与全部在线 fleet 为 `0.1.237`；
离线 `blg` 按当前决策保持 `0.1.175`。LAE staging 的 9 个 task、wildcard DNS-01 TLS 与 Web/API/Agent/artifact
...
已升级到 `0.1.237`，LAE 平台为 Job v54；升级无需人工重启。升级中心会在变更前保存
健康路由集合，Control 恢复后对同一集合复放，本轮 11/11 成功。更长时间的多 edge
...
`0.1.237` route sentinel 只从当前 Nomad service 与 active deployment 构造 inventory，
