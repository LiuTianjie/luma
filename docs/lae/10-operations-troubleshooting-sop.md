...
截至 2026-07-12，Luma CLI、Control 与 live fleet 已统一到 `0.1.171`；该 release
曾通过 719/719 gate。LAE staging 的 9 个 task、TLS 与 Web/API/Agent/artifact 探针
健康，Agent ready 显示 AI provider 已配置；Mailpit 注册/token/CLI/template/analysis
已跑，但真实邮箱送达、最新 provider-backed verdict 与 tenant Runtime
deploy/lifecycle 仍待最终 E2E。当前 working tree 的 rollout/route 可靠性修复尚未形成
新 release，不能按 live 能力处理。
...
当前 staging 的构建分发不变量是：其它节点使用
`registryHost=100.66.177.70:5000` 拉取，builder 本机使用
`pushHost=localhost:5000` 推送。两者职责不同，不能为了“统一”改成同一个值。
...
1. 先确认 manager、Control、builder agent 都是同一已发布候选版本，且 health capability
   包含 `build-proxy-mode-v1`。旧版本先按部署升级手册修复，不能在旧 Builder 上
   反复 retry。
...
- build 日志的 clone → base pull → dependency install → push 顺序闭合，未出现旧
  proxy 地址；
- builder 本机能从 `localhost:5000` 查到 digest，`lab`、`manager`、`tecent` 能从
  `100.66.177.70:5000` 按同一 digest 拉取；
...
当前 live `0.1.171` 已出现过这样的组合故障：升级 Control、更新 Docker
registry/proxy 配置或发布另一个应用后，若 Docker daemon 被重启，既有 Nomad
allocation 的 CNI/network namespace 可能丢失；容器和 allocation 仍显示
`running`，但 upstream 已不可达。与此同时，route reconciliation 若未收敛会表现为
Traefik 404。手工 recreate allocation 可以恢复症状，但不代表根因已关闭。
...
4. 尚未发布的 agent 诊断会在
   ...
   interface；严格只有 `lo` 时才报告 `allocId/container/name/interfaces`。Live
   `0.1.171` 没有该字段是预期状态；字段为空也只是 fail-safe“未发现”，因为无 init
...
**Safe action — 当前 live 的 staging 恢复**
