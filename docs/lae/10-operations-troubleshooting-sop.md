...
> 安全默认：先只读、再隔离 validation、最后才做经过批准的生产变更。本 SOP 中的命令不会执行部署；带写入效果的命令只用于明确标注的 validation 恢复步骤。
...
- builder-only 和未显式 opt-in 的 control-plane 节点继续被排除；当前 validation 的 manager 只有同时带 runtime role 且位于 allowlist 才可进入；
...
Control state 可以保存包含 node ID 的内部 placement 决策；租户投影明确省略它。当前 Luma Dashboard 的 LAE 页面已提供跨租户用户、应用、Operation、用量和“调度位置”只读视图；placement 页会把内部候选、preferred node 与实时 Nomad allocation 关联。该视图和 API 只接受 Luma management token，节点拓扑不得进入租户工单。真实 validation 中的 RBAC、allocation 关联和故障态展示尚未验收。

当前实现已经具备正向 node allowlist、region/readiness/capability、builder-only/control-plane policy、volume compatibility、prior affinity 和 Nomad plan；validation bundle 默认 allowlist 为 `manager + tecent`。manager 只有显式 `runtime` role 才能进入，移除该 role 或 allowlist 项都会 fail closed。生产应换成至少两个专用 runner；通用 `docker-image` capability 不能单独证明节点适合不可信租户 workload。

本轮真实 validation 的内部拓扑以此为准：`manager` 是唯一控制面，同时是经过显式
opt-in 的 runtime candidate；LAE 平台 10 个服务与平台本地卷固定在 `manager`；租户 runtime 候选为
...
截至 2026-07-14，本机 Luma CLI、Control、manager agent 与全部在线 fleet 为 `0.1.251`；
离线 `blg` 按当前决策保持 `0.1.175` 且未触碰。LAE validation 的 10 个 service、wildcard DNS-01 TLS 与 Web/API/Agent/artifact
探针健康；当前 live 平台仍是回滚后的 exact ref `35591c4e789f7d7bec60614d427fed05023b373a`。四服务 Compose 双 HTTPS/双持久卷产品 E2E 与 clean-room CLI/Skill E2E
...
**Safe action — 仅限已批准的 validation 恢复**
...
- base image 拉取极慢、`unexpected EOF`、`short read` 或 `ECONNRESET`；
- `luma import` 能 clone，但 build、push 或 runtime pull 在不同阶段失败；
- 使用完整 commit 时出现 `Remote branch <40-hex> not found`；
- BuildKit 把内部 HTTP registry 错当成 HTTPS；
- build 已 push，但 runtime candidate 无法按 digest 拉取；
- 长构建只有 `Build image: started`，无法区分排队、仍运行或已经失联。

**当前架构合同**

Repository Import 和 LAE 的源码构建统一在 `builder` 执行，构建产物统一写入
`builder` 自己托管的内部 registry。实时 Control 配置中的 `build.registryHost` 与
`build.pushHost` 必须相同；当前 validation 都是 `100.66.177.70:5000`。registry 的
allocation、监听和持久数据都在 `builder`，不允许迁到 manager，也不经过公网
`registry.itool.tech`。后者是历史入口，不是新构建的 canonical registry。

必须区分两类镜像语义：

1. `luma import` / LAE Git 构建中的 `build:` 服务由 Builder 构建，Control 把最终
   runtime image 固定为内部 registry 的 immutable digest；Compose 中用于触发构建的
   本地占位 tag 不能原样下发到 runtime。
2. 用户在 `luma deploy`、普通 Compose 或 LAE 计划中**明确指定**的外部/私有
   `image:` 保持原地址并直接拉取；私有 registry 通过对应 credential 配置鉴权，不能
   被偷偷改写到内部 registry。只有显式 import build 才使用上述产物重写。

Builder 拉外部 base image 的网络策略属于 Builder task；runtime 拉 Builder 产物走
tailnet 内部 registry。部署某个租户应用时不得临时修改目标节点的全局 Docker proxy，
也不得把 manager 当作镜像中继。内部 registry 地址必须长期存在于相关 Docker/
BuildKit 的 `NO_PROXY` 和 insecure-registry 配置中，这些是 Luma 节点配置状态，不是
每次部署时切换的临时状态。
...
从 Luma Dashboard 的节点诊断或受审计的 root terminal，在 `builder` 和每个实际
runtime candidate 分别请求内部 `/v2/`；再核对 Docker daemon 的
`insecure-registries`/`NO_PROXY` 与 BuildKit builder 配置。宿主 dockerd 和
docker-container driver 内的 BuildKit 是两个配置域，不能用一次 `curl` 成功推断
Docker pull/push 一定正确。任何历史 `aly` 地址、manager proxy、
`registry.itool.tech` 构建目标或内部 registry 不在 `NO_PROXY` 都是配置漂移。

构建日志必须持续包含真实 buildx 输出；安静阶段从 `v0.1.250` 起每 15 秒记录一次
带 elapsed seconds 的安全 heartbeat。heartbeat 只证明子进程仍存活，不代表下载速率
或阶段完成。排队任务的等待时间与实际执行超时分别计算，不能因为前序大构建占用
Builder 而消耗后续任务的执行预算。

**Safe action — 仅限已批准的 validation 恢复**

1. 先确认 Control 与 `builder` agent 是本次发布版本，且 Builder capability 满足当前
   task。完整 commit 的 detached fetch、内部 registry push 和长构建 heartbeat 都应
   通过正式升级获得，不能靠远程 shell 临时补丁。
2. 只读确认 registry allocation 与 storage 仍在 `builder`。如果 registry 本身健康，
   不得删除/recreate、迁到 manager、切公网入口或清理 blob。
3. 如果 Control 的 Builder 地址漂移，重新声明唯一正确配置：
...
4. 只有 registry service 缺失且已确认原数据目录/volume 可复用时，才使用
   `luma registry serve --node builder ...` 恢复同一个内部端点；默认保留存储，禁止
   删除 registry 数据。恢复后重新执行第 3 步。
5. BuildKit 配置漂移时，先等待 active build 终态，再让 Luma 的版本化节点配置修复；
   不要在构建中途重启 Docker、删除全部缓存或手工改全局 proxy。
6. 外部私有 `image:` 拉取失败时，修复对应 registry credential/网络，不要把镜像
   当成 import 产物改写。Git/Compose 中的 `build:` 产物失败才进入内部 registry 路径。

**Verify**

- build 日志的 clone → base pull → dependency install → push 顺序闭合；安静阶段有
  heartbeat，终态明确；
- 完整 commit import 的 resolved commit 与请求逐字相等，checkout 为 detached；
- push 目标是 `100.66.177.70:5000/...@sha256:...`，没有 manager 或公网 registry；
- `builder` 与目标 runtime candidate 能按同一 digest 查找/拉取；
- import 生成的 runtime job 使用 immutable 内部 digest；用户明确写出的外部
  `image:` 地址保持不变；
- `luma status --format json` 的 `build.registryHost` 与 `build.pushHost` 都是
  `100.66.177.70:5000`。

**Escalate**

版本化配置修复后仍复用旧代理、registry digest 不一致、内部 registry 流量离开
tailnet、或只有删除共享缓存/blob 才能继续时，停止 import lane 并升级 Luma
Builder/供应链负责人。
...
- digest 漂移时先构建并校验新的 immutable analyzer image，再按顺序更新 Builder agent、
  Control allowlist 和 LAE validation bundle；三方未一致前停止新的 analyze lane，不要扩大
...
- 计划误报时修复稳定 seed，并增加“两个等价 source snapshot 产生相同计划”的回归
  ...
  revision 做一次批准的 validation 重部署来刷新基线；不能直接改数据库 digest。
...
本轮 validation 的验证样本最终收敛到 plan digest
...
- URL/timeout 错误需要 Luma Control 配置变更时先在 validation 更新并验证，再走 manager 变更流程。
- 轮换必须同时协调 LAE API 端与 Luma Control 端，见第 15 节；单边轮换会造成短时全失败。
- Control 仍可读且只是一个 validation task异常时，优先恢复该 task，不重新 bootstrap 整个 manager。
...
2. 在 Luma Dashboard 查看 `lae-platform[-validation]` 的 `web`、`api` task 状态、最近 events、image digest、日志和资源使用。
...
- 只有 validation 的 API/Web 进程明确卡死且依赖健康时，使用窄重建：
...
### 6.1 模板每日 smoke、自动下架与恢复

300 秒，让 API migration 和 Worker lease 完成收敛。它不是“探一下首页”：每个模板都
创建临时应用，执行正常 Agent analysis、配置、Builder build、Runtime deploy、公开
HTTPS probe 和删除清理。结果通过独立 `LAE_TEMPLATE_SMOKE_REPORT_TOKEN` 写入
PostgreSQL `template_health`；普通 deploy token、用户 session 和 Luma management
token 都不能写该端点。

**Checks**

- 检查 task 仍为 healthy，heartbeat 不超过 48 小时；
- 读取 task 日志中的 `template_smoke_started/succeeded/failed/complete`，不要只看
  scheduler 容器 running；
- `/v1/templates` 只能看到已发布模板和 `verification.healthStatus`，不得出现
  `publication`、失败计数或内部 token；
- 同一个 `runId + templateId + version` 的重复报告必须返回同一状态，不增加失败计数；
- 调度器自身无法连接报告端点时整轮失败，但不能把报告网络故障改写成模板失败。

**Auto action**

- 单次、两次连续失败只记录状态；第三次连续失败把该模板版本从公开 catalog 下架；
- smoke 使用普通 deploy token 加独立 smoke token 的双重鉴权，因此下架后仍可验收；
- 后续一次完整成功把计数清零并自动恢复发布；模板版本变化从新的未验证状态开始；
- 恢复必须来自真实 smoke 成功，值班人员不得直接改表或伪造成功报告。

**Escalate**

同一模板首次失败先按 analysis/build/runtime/route 阶段归因；多个模板同轮失败优先按
Builder、Runtime、DNS/TLS 或报告链路平台事故处理。`template-smoke` heartbeat 过期、
清理残留应用、连续自动上下架或 token 认证失败时停止新增模板发布并升级 LAE 值班人。
...
1. 查看 Luma Dashboard 中 `postgres` task、allocation events、健康检查、内存/OOM 和实际挂载。当前 validation 平台 PostgreSQL 是 manager 本地 bind path，不属于 tenant runtime storage class。
...
- Analyzer digest 缺失/不一致时先停止新 analysis；从批准的构建记录取得 immutable repo digest，推送到 Builder 可拉取 registry、在目标 Builder 预拉，再同步更新 Worker/Control 的两个配置。先在 validation 验证，不把 executor 改成在线 `pull always`。
...
3. 确认 builder-only、core/stateful 和未显式 runtime opt-in 的控制面节点没有被误纳入；当前 validation 的 manager 应同时满足 runtime role 与 allowlist 两个条件。
...
6. 授权管理员优先在 Luma Dashboard 的 LAE →“调度位置”查看 runtime deployment、tenant/application、region、候选、preferred node、实时 allocation 和 continuity。该页面使用 Luma management token，只能留在内部审计上下文；真实 validation 尚未验证时，空表不代表 placement 不存在。
...
- 内部 `capacity_unavailable`：先恢复/扩容正确 region 的 runtime pool，或减少下一 revision 的资源；不把 workload 临时调度到 builder，也不把未显式 opt-in 的控制面节点加入 allowlist。当前 manager 已是批准的 validation runtime，不应绕过 Nomad plan 强制落点。
...
6. 若 `artifact-init` 退出 `137`、Nomad event 为 OOM 或 API/Worker 一直等待它，
   检查 rendered task 资源。真实 validation 已证明 `128M` 会触发 OOM；当前硬底线为
...
`luma_tailscale_ip` metadata 注册 edge upstream。当前 Control、manager 与全部在线 fleet
已升级到 `0.1.251`；manager 与六个在线节点升级无需人工重启。升级中心会在变更前保存
健康路由集合，Control 恢复后对同一集合复放，历史基线 11/11 成功；最终 manager 更新期间 LAE Web/API/Agent/Artifact 与 Gateway 连续 5 轮均为 200，更新后 60 秒各 14/14。更长时间的多 edge
探针和 Docker/CNI 故障注入仍是 production gate，因此仍不能宣称生产“零中断”。

2026-07-14 的 LAE 平台失败 rollout 又暴露了未关闭的 P0：同一 10-service Nomad group
在 progress deadline 失败/回滚时，四条 LAE route 曾同时出现 404/502；自动回滚后的
route cleanup 还会覆盖已恢复的 route，需要在 build run 到达终态后再次执行 exact
version rollback 才恢复 4/4 HTTP 200。因此不能再声称正常发布路径已完全关闭该问题。
后续必须让 route ownership 与 job version/rollout transaction 绑定，并保证失败清理不
删除前一稳定版本的 route，再补失败 rollout sentinel 回归。

Docker daemon restart 后的 CNI 自愈和 route reconciliation 故障注入也仍未完成。手工 recreate allocation 仍只能作为保留证据后的
validation 恢复动作，不能重新成为发布协议。
...
当前 route sentinel 只从当前 Nomad service 与 active deployment 构造 inventory，
不再盲扫遗留 route 文件；应用实际返回的 JSON/HTML 404 证明 router/upstream 已工作，
可判为 route healthy，而 Traefik 纯文本默认 `404 page not found` 仍判失败。升级门禁不以
“升级后重新扫描全量 route”为基线，而是复放升级前已健康的同一 domain 集合。完整
inventory 仍会报告既有坏应用，它只能作为发现信号，不能把历史故障误归因于本次升级。
...
3. validation preview 只允许保留的 `.invalid` 身份；普通邮箱必须检查 SMTP/API provider 的真实发送结果，不能从 preview 接口取验证码。
4. 检查 provider credential、host/465/TLS、发件人、退信/限流、SPF/DKIM/DMARC 和 DNS；只有真实 canary 收件成功才能判定通道可用。
5. 检查验证码 TTL、失败次数锁定、设备/IP 限流和服务器时间。

**Safe action**

- 只在 validation 使用专用 canary 邮箱执行一次注册/登录 synthetic；不要使用真实用户邮箱，也不把 OTP 复制到工单。
- provider 限流时降低发送速率并保持统一响应，不关闭 abuse limit。
- 配置错误通过 secret manager 修复后先发 validation canary，再生产灰度。
- 不读取用户邮箱，不要求用户提供完整邮件链接。

**Verify**

- validation canary 在 TTL 内送达；
...
1. 确认环境：production 当前应为 `LAE_BILLING_DRIVER=disabled`；validation 才允许 `mock`。生产 disabled 返回 503 是正确门禁，不是支付成功率事故。
2. 使用 Luma Dashboard LAE 的 tenant/usage/operation 只读视图检查 plan、订单相关 Operation 和当前用量；不读取支付 secret。
3. validation 用专用 tenant 查看：
...
- mock 只在 validation 由人工完成 checkout；Agent 不代付。
...
- 完成一次从 backup 到可用 validation 的实际 drill，不只检查文件存在。
...
- **Builder/Runtime principal**：创建全新随机 token file，保持 config/token regular `0600`，先在 validation 协调 LAE 与 Luma 两端，再原子切换；两种 principal 永远不同。
...
- 所有动作先 validation；生产需要变更批准和回退计划。
...
lifecycle Worker 未启用/配置失败、状态机卡死、current pointer 错误、重复动作并发或 volume policy 不明确时停止该应用后续写动作，升级 LAE lifecycle/runtime 负责人。当前 executor 代码、自动化测试与 PostgreSQL 17 migration-backed 集成已验证；真实 Luma validation 的故障恢复证据仍是发布门禁。
...
`scripts/setup-lae-builder.sh` 使用 Bash 语法，必须由 `bash -n` 检查；其余列出的容器脚本是 POSIX `sh`，使用 `sh -n`。若 workspace Python 路径不同，以仓库当前 CI/Makefile 为准。自动化测试和本地完整 validation Compose E2E 通过仍不等于 `Verified`；真实 Luma validation 还必须覆盖多 HTTP Compose、私有 Git、HTML/ZIP、无容量、节点故障重调度、volume affinity、route/TLS、cancel/orphan 和 backup/restore drill。
