...
离线 `blg` 按当前决策保持 `0.1.175` 且未触碰。LAE staging 的 10 个 service、wildcard DNS-01 TLS 与 Web/API/Agent/artifact
...
`build.pushHost` 必须相同；当前 staging 都是 `100.66.177.70:5000`。registry 的
...
`lae-platform-staging` 的 `template-smoke` task 每 86400 秒执行一次，启动后先等待
...
1. 查看 Luma Dashboard 中 `postgres` task、allocation events、健康检查、内存/OOM 和实际挂载。当前 staging 平台 PostgreSQL 是 manager 本地 bind path，不属于 tenant runtime storage class。
...
3. Staging preview 只允许保留的 `.invalid` 身份；普通邮箱必须检查 SMTP/API provider 的真实发送结果，不能从 preview 接口取验证码。
