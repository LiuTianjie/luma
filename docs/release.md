...
```bash
python -m pip install "luma-infra==0.1.74"
```

Interactive users can still install with:

```bash
curl -fsSL https://raw.githubusercontent.com/LiuTianjie/luma/main/scripts/install-luma.sh | LUMA_INSTALL_REF=v0.1.74 sh
```
...
defaults:
  images:
    lumaControl: ghcr.io/liutianjie/luma-control:v0.1.74
```
