...
- 在 **控制台 → 可观测性 → 应用 → Traces** 或 Grafana `/grafana/d/luma-traces` 查看。
...
**控制台 → 可观测性**默认打开告警事件。指标有独立页面；日志通过 Grafana Explore 查询 VictoriaLogs（保留 7 天）。规则与通知渠道分别有列表和编辑地址。存储治理位于 **基础设施 → 存储 → 数据治理**。
