...
```bash
python -m pip install "luma-infra==0.1.286"
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/LiuTianjie/luma/main/scripts/install-luma.sh | LUMA_INSTALL_REF=v0.1.286 sh
```
...
```bash
luma update fleet
luma update fleet --install-ref v0.1.286 --timeout 900
