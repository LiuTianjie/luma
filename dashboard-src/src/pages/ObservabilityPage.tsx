...
export function ObservabilityPage({ lang, token, vm }: { lang: Lang; token: string; vm: DashboardViewModel }) {
  ...
  const tabs = [["incidents", "/observe", zh ? "告警事件" : "Incidents"], ["apps", "/observe/apps", zh ? "应用" : "Apps"], ["metrics", "/observe/metrics", zh ? "资源指标" : "Metrics"], ["logs", "/observe/logs", zh ? "实时日志" : "Logs"], ["rules", "/observe/rules", zh ? "告警规则" : "Rules"], ["channels", "/observe/channels", zh ? "通知渠道" : "Channels"]];
  return <>
    <PageHeader meta={{ eyebrow: zh ? "运行保障" : "Operations", title: zh ? "可观测性" : "Observability", metrics: [], description: section === "apps" ? "" : (zh ? "从告警定位对象，查看指标与实时日志。" : "Investigate incidents with resource metrics and live logs.") }} />
    ...
    {section === "apps" ? <ObserveAppsPanel lang={lang} token={token} applicationNames={vm.applications.map((app) => app.stack)} /> : (section === "metrics" || section === "logs") ? <ObservabilityPanel lang={lang} token={token} nodes={vm.nodes} services={vm.services} mode={section} /> : <AlertingPanel key={token} lang={lang} token={token} tab={section === "rules" ? "rules" : section === "channels" ? "notifications" : "alerts"} nodeNames={vm.nodes.flatMap((node) => node.name ? [node.name] : [])} applicationNames={vm.applications.map((app) => app.stack)} />}
  </>;
}
