...
export function StorageGovernancePanel({ lang, token }: { lang: Lang; token: string }) {
  ...
  return <div className="storage-governance">
    ...
    <article className="panel">
      <div className="panel-heading"><div><p className="eyebrow">MANAGER</p><h2>{txt("存储占用与增长", "Storage usage and growth")}</h2></div><Button variant="outline" disabled={Boolean(busy)} onClick={() => setRevision((v) => v + 1)}><RefreshCw size={14} />{txt("刷新", "Refresh")}</Button></div>
      {error ? <Alert><AlertCircle /><AlertTitle>{txt("提示", "Notice")}</AlertTitle><AlertDescription>{error}</AlertDescription></Alert> : null}
    ...
    <article className="panel"><div className="panel-heading"><div><p className="eyebrow">RETENTION</p><h2>{txt("历史保留策略", "History retention")}</h2></div></div>
      {policy ? <form onSubmit={(event) => { event.preventDefault(); void action("policy", async (signal) => { const next = await saveStoragePolicy(token, policy, signal); setPolicy(next); setDirty(false); setPlan(null); setMessage(txt("策略已保存。清理前请重新生成预览。", "Policy saved. Generate a fresh preview before cleanup.")); }); }}>
        ...
        <div className="governance-actions"><Button type="submit" disabled={Boolean(busy)}>{txt("保存策略", "Save policy")}</Button><Button variant="outline" type="button" disabled={Boolean(busy) || dirty} onClick={() => void action("preview", async (signal) => setPlan(await previewHistoryCleanup(token, signal)))}>{txt("预览历史清理", "Preview history cleanup")}</Button></div>
    ...
    <article className="panel"><div className="panel-heading"><div><p className="eyebrow">BUILDER</p><h2>{txt("远端产物盘点与回收", "Remote artifact inventory and reclamation")}</h2></div></div>
      ...
      <div className="governance-actions"><Field className="min-w-64">
        <FieldLabel>{txt("Builder 节点", "Builder node")}</FieldLabel>
        <SelectControl
          className="min-w-0"
          disabled={taskRunning || Boolean(busy)}
          value={node}
          onChange={(value) => { setNode(value); setTask(null); setResult(null); }}
          options={[
            { value: "", label: txt("选择节点", "Select node") },
            ...(inventory?.builders || []).map((builder) => ({ value: builder.name, label: builder.status ? `${builder.name} · ${builder.status}` : builder.name })),
          ]}
        />
      </Field><Button variant="outline" disabled={!node || Boolean(busy) || taskRunning} onClick={() => void remote("inventory")}>{txt("盘点占用", "Inspect usage")}</Button><Button disabled={!node || Boolean(busy) || taskRunning} onClick={() => void remote("preview")}>{txt("生成清理预览", "Preview cleanup")}</Button></div>
      {inventory?.builderTasks?.some((item) => item.nodeName === node && item.result?.planId) ? <div className="governance-actions"><Field className="min-w-64">
          <FieldLabel>{txt("重新打开 Builder 计划", "Reopen a builder plan")}</FieldLabel>
          <SelectControl
            className="min-w-0"
            disabled={Boolean(busy) || taskRunning}
            value={task?.id || ""}
            onChange={(value) => { const saved = inventory.builderTasks?.find((item) => item.id === value); if (saved) { setTask(saved); setResult(saved.result || null); } }}
            options={[
              { value: "", label: txt("选择计划任务", "Select a plan task") },
              ...inventory.builderTasks.filter((item) => item.nodeName === node && item.result?.planId).map((item) => ({ value: item.id, label: `${item.result?.planId?.slice(0, 12)} · ${item.result?.operation} · ${date(item.updatedAt)}` })),
            ]}
          />
        </Field></div> : null}
