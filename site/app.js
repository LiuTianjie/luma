...
if (demoStage && demoButtons.length) {
  ...
  const demoCopy = {
    install: {
      ...
      domain: isZh ? "本机可执行 luma" : "local luma command ready",
      domainStatus: isZh ? "后续 manager、worker、client 都用同一个命令" : "manager, workers, and clients use the same command",
      dns: isZh ? "未配置" : "not configured",
      dnsStatus: isZh ? "安装阶段不碰 DNS" : "install does not touch DNS",
      tls: isZh ? "未申请" : "not requested",
      tlsStatus: isZh ? "等控制面和服务域名创建后再签发" : "issued after control and service hosts exist",
      stack: isZh ? "无工作负载" : "no workload",
      stackStatus: isZh ? "此时只是准备好 CLI" : "this step only prepares the CLI",
    },
    bootstrap: {
      ...
      dnsStatus: isZh ? "控制面入口先可访问" : "the control endpoint is reachable first",
      tls: isZh ? "控制面证书" : "control certificate",
      tlsStatus: isZh ? "Traefik 为控制面签发 HTTPS" : "Traefik issues HTTPS for control",
      stack: isZh ? "控制面服务运行" : "control services running",
      stackStatus: isZh ? "后续部署都提交给这台 manager" : "later deploys are submitted to this manager",
    },
    "join-cn": {
      ...
      global: isZh ? "等待加入" : "waiting to join",
      client: isZh ? "无需登录 worker" : "no worker login needed",
      domain: isZh ? "国内服务可选 cn-edge" : "CN services can target cn-edge",
      domainStatus: isZh ? "服务 manifest 用 exposure 选择入口节点" : "manifest chooses the entry node with exposure",
      dns: isZh ? "等待服务域名" : "waiting for service domain",
      dnsStatus: isZh ? "真正 deploy 时才写业务 DNS" : "service DNS is written during deploy",
      ...
      stackStatus: isZh ? "工作负载可以落到国内节点" : "workloads can be placed on the CN node",
    },
    "join-global": {
      ...
      stackStatus: isZh ? "控制面知道哪些节点适合承载负载" : "control knows where workloads can run",
    },
    deploy: {
      ...
      client: isZh ? "提交域名服务" : "submitted named service",
      domain: "status.example.com",
      domainStatus: isZh ? "manifest 声明服务域名和端口" : "manifest declares service domain and port",
      dns: "status.example.com -> cn-edge",
      dnsStatus: isZh ? "Cloudflare 自动指向 cn-edge" : "Cloudflare automatically points to cn-edge",
      tls: isZh ? "HTTPS 证书签发中" : "HTTPS certificate issuing",
      tlsStatus: isZh ? "Traefik 根据 Host 规则完成 ACME" : "Traefik completes ACME from the Host rule",
    ...
    published: {
      ...
      domainStatus: isZh ? "用户访问域名就进入服务" : "users reach the service through the domain",
      dns: "status.example.com -> cn-edge",
      dnsStatus: isZh ? "DNS、证书、路由都已生效" : "DNS, certificate, and route are active",
      tls: isZh ? "HTTPS 可用" : "HTTPS active",
      tlsStatus: isZh ? "无需手动申请或续期证书" : "no manual certificate request or renewal",
