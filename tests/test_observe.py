...
class ObserveTraceStackTests(unittest.TestCase):
    ...
    def test_apps_traces_tab_embeds_tempo_explore_without_kiosk(self):
        ...
        self.assertIn("<SelectControl", text)
        self.assertIn('value={app}', text)
        self.assertIn('onChange={setApp}', text)
        self.assertIn('htmlFor={`${fieldId}-app`}', text)
        self.assertIn('id={`${fieldId}-app`}', text)
