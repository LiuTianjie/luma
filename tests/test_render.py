...
"""
    ...
    def test_database_volume_uses_storage_service_without_workload_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "docker-compose.yml").write_text(
                yaml.safe_dump(
...
"""
    ...
    def test_service_database_volume_uses_storage_service_without_workload_gate(self):
        service = self.load(
            """
name: postgres
image: postgres:16-alpine
...
"""
        ...
        rendered = yaml.safe_load(render_stack(self.config(), service, storage_classes=storage_classes))
        self.assertEqual(rendered["volumes"]["pg-data"]["driver_opts"]["device"], ":/srv/luma/postgres/pg-data")
...
"""
        ...
        self.assertEqual(app["ports"][0]["mode"], "host")
        route = yaml.safe_load(render_tcp_route(self.config(), service))
        self.assertEqual(route["tcp"]["routers"]["granary-db"]["rule"], "HostSNI(`*`)")
        self.assertEqual(route["tcp"]["routers"]["granary-db"]["entryPoints"], ["tcp-3306"])
        self.assertEqual(
            route["tcp"]["services"]["granary-db"]["loadBalancer"]["servers"][0]["address"],
            "lab:3306",
        )

    def test_tcp_relay_entrypoint_uses_publish_port(self):
        service = self.load(
            """
name: granary db
image: mysql:8.4.9
...
"""
        ...
        route = yaml.safe_load(render_tcp_route(self.config(), service))
        self.assertEqual(route["tcp"]["routers"]["granary-db"]["entryPoints"], ["tcp-13306"])
        self.assertEqual(
            route["tcp"]["services"]["granary-db"]["loadBalancer"]["servers"][0]["address"],
            "lab:13306",
        )
...
""",
            ...
            self.assertEqual(route["tcp"]["routers"]["granary-mysql"]["entryPoints"], ["tcp-3306"])
            self.assertEqual(
