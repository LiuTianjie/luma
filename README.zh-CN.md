...
| 你希望 client 机器只拿管理 Token，不持有 SSH、Docker、Cloudflare 或 Portainer 凭据。 | 你完全不想使用公网域名或 Cloudflare DNS。 |
...
只有当服务必须固定在某个 Luma 节点名上时，才在 manifest 里设置 `node`；Luma 仍然会保留 `region` 调度约束，并在部署时解析成真实 Swarm NodeID。
...
Bootstrap 会安装/检查 Docker，初始化 Swarm，创建 overlay network，部署 Traefik、Portainer、Luma Control，配置防火墙，并按需设置 egress。它会输出管理 Token 和节点加入 Token。
...
默认会离开 Swarm 并清理 `/opt/luma` 下的本地 Luma runtime 状态，但保留 Tailscale 和 Docker image/volume cache。需要同时从控制面注销节点名时加 `--endpoint <control-url> --token <management-token-or-node-join-token>`；需要同时退出 Tailscale 时加 `--tailscale`；明确想清理未使用 Docker 缓存和 volume 时才加 `--prune-docker`。

如果只是清理控制面中残留的 registered-only 记录，可以在已登录 client 上运行 `luma node remove <name>`。
