...
默认会离开 Swarm 并清理 `/opt/luma` 下的本地 Luma runtime 状态，但保留 Tailscale 和 Docker image/volume cache。需要同时从控制面注销节点名时加 `--endpoint <control-url> --token <token>`；需要同时退出 Tailscale 时加 `--tailscale`；明确想清理未使用 Docker 缓存和 volume 时才加 `--prune-docker`。
